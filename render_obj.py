...
def distort_image(img, camera_matrix, distortion):
    height, width = img.shape[:2]  # Swap width and height
    ...
    pts_distort = np.stack((x_coords.ravel(), y_coords.ravel()), axis=1).astype(dtype=np.float32)

    pts_ud = cv2.undistortPoints(pts_distort.reshape(-1, 1, 2), camera_matrix, distortion)
    pts_ud = pts_ud.reshape(height, width, 2).astype(dtype=np.float32)
...
if __name__ == "__main__":
    ...
    if GENERATE_IMAGES:
        for current_info in camera_info["image_data"]:
            ...
            if DISTORT_OPENGL:
                rendered_image = distort_image(original_image, camera_info["camera_matrix"], camera_info["distortion"])
